...
private:
  ...
  float W_leptonic_mass, W_hadronic_mass, top_hadronic_mass_1,
      top_hadronic_mass_2, top_leptoninc_mass;
};
...
void CreateTuple::setBranchesAddressesOutput() {
  ...
  tree_output->Branch("N_valid_jets_tot", &N_valid_jets_tot, "N_valid_jets_tot/i");
  tree_output->Branch("diMuon_mass", &diMuon_mass, "diMuon_mass/f");
  tree_output->Branch("weight", &weight, "weight/f");

  tree_output->Branch("W_leptonic_mass", &W_leptonic_mass, "W_leptonic_mass/f");
  tree_output->Branch("W_hadronic_mass", &W_hadronic_mass, "W_hadronic_mass/f");
...
void CreateTuple::fillOutputTree(TString channel) {
  ...
  for (int event_index = 0; event_index < total_entries; event_index++) {
      ...
      for (int i = 0; i < NMuon && !found_pair; ++i) {
        for (int j = i + 1; j < NMuon && !found_pair; ++j) {
          if (Muon_Charge[i] * Muon_Charge[j] == -1) {
            ROOT::Math::PxPyPzEVector mu1(Muon_Px[i], Muon_Py[i], Muon_Pz[i],
                                          Muon_E[i]);
            ROOT::Math::PxPyPzEVector mu2(Muon_Px[j], Muon_Py[j], Muon_Pz[j],
                                          Muon_E[j]);
    ...
    // if (NMuon > 1) {
    // int pair_idx = -1;
    // if (Muon_Charge[0] * Muon_Charge[1] == -1) {
    // pair_idx = 1;
    //} else {
    // if (NMuon > 2) {
    // if (Muon_Charge[0] * Muon_Charge[2] == -1) {
    // pair_idx = 2;
    //} else {
    // muon_idx = 1;
    // pair_idx = 2;
    ...
    //}
    //}
    // ROOT::Math::PxPyPzEVector diMuon_vector;
    // ROOT::Math::PxPyPzEVector mu1_vector(Muon_Px[muon_idx],
    // Muon_Py[muon_idx], Muon_Pz[muon_idx], Muon_E[muon_idx]);
    // ROOT::Math::PxPyPzEVector mu2_vector(Muon_Px[pair_idx],
    // Muon_Py[pair_idx], Muon_Pz[pair_idx], Muon_E[pair_idx]);
    // diMuon_mass = (mu1_vector + mu2_vector).M();
    //} else {
    // diMuon_mass = -1;
    //}
    ...
    auto fill_electron = [&](int i, float &px, float &py, float &pz, float &e,
                             float &iso, float &pt, int &n_valid) {
      if (NElectron > i) {
        px = Electron_Px[i];
        py = Electron_Py[i];
    ...
    fill_muon(0, mu1_Px, mu1_Py, mu1_Pz, mu1_E, mu1_Iso, mu1_Pt, NMuon_valid);
    fill_muon(1, mu2_Px, mu2_Py, mu2_Pz, mu2_E, mu2_Iso, mu2_Pt, NMuon_valid);
    fill_muon(2, mu3_Px, mu3_Py, mu3_Pz, mu3_E, mu3_Iso, mu3_Pt, NMuon_valid);
    ...
    if (channel == "ttbar") {
      ...
      ROOT::Math::PxPyPzMVector MC_lepton_vector(MClepton_px, MClepton_py,
                                                 MClepton_pz, MC_lepton_mass);
      ROOT::Math::PxPyPzMVector MC_nu_vector(MCneutrino_px, MCneutrino_py,
                                             MCneutrino_pz, NU_MASS);

      MCtop_mass_leptonic =
          (MC_B_leptonic_vector + MC_nu_vector + MC_lepton_vector).M();

      ROOT::Math::PxPyPzMVector MC_q_vector(MChadronicWDecayQuark_px,
                                            MChadronicWDecayQuark_py,
    ...
    for (int jet = 0; jet < NJet; ++jet) {
      ...
      // if (!Jet_ID[jet] || std::hypot(Jet_Px[jet], Jet_Py[jet]) > 20)
      if (!Jet_ID[jet])
        continue; // usar solo jets buenos
    ...
    W_leptonic_mass = W_hadronic_mass = top_hadronic_mass_1 =
        top_hadronic_mass_2 = top_leptoninc_mass = -1;
    ...
    for (size_t idx1 = 0; idx1 < valid_jets_idx.size(); ++idx1) {
      for (size_t idx2 = idx1 + 1; idx2 < valid_jets_idx.size(); ++idx2) {
    ...
    ROOT::Math::PxPyPzEVector b1_vector(Jet_Px[bj1], Jet_Py[bj1], Jet_Pz[bj1],
                                        Jet_E[bj1]);

    top_hadronic_mass_1 = (W_hadronic_vector + b1_vector).M();
    int bj2 = valid_b_jets_idx[1];
    ROOT::Math::PxPyPzEVector b2_vector(Jet_Px[bj2], Jet_Py[bj2], Jet_Pz[bj2],
                                        Jet_E[bj2]);

    top_hadronic_mass_2 = (W_hadronic_vector + b2_vector).M();
    // double hadronic_top_mass_diff = 1e9;
